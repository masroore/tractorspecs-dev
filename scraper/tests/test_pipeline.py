...
class TestUpsertManufacturer:
    async def test_returns_id_on_insert(self, conn) -> None:
        ...
        mfr_id = await upsert_manufacturer(conn, {"name": "TestCo", "slug": "testco", "url": "/tractors/testco/"})
    ...
    async def test_idempotent(self, conn) -> None:
        ...
        data = {"name": "IdempotentCo", "slug": "idempotentco", "url": "/tractors/idempotentco/"}
...
class TestUpsertSeries:
    async def test_returns_id(self, conn) -> None:
        ...
        mfr_id = await upsert_manufacturer(conn, {"name": "SeriesCo", "slug": "seriesco", "url": "/tractors/seriesco/"})
    ...
    async def test_idempotent(self, conn) -> None:
        ...
        mfr_id = await upsert_manufacturer(conn, {"name": "SeriesCo2", "slug": "seriesco2", "url": "/tractors/seriesco2/"})
...
class TestUpsertModel:
    async def test_returns_id_and_needs_update_on_first_insert(self, conn) -> None:
        ...
        mfr_id = await upsert_manufacturer(conn, {"name": "ModelCo", "slug": "modelco", "url": "/tractors/modelco/"})
        series_id = await upsert_series(conn, mfr_id, {"name": "M Series", "slug": "m-series", "models": []})
        model_data = {
            "name": "ModelCo M1",
            "slug": "modelco-m1",
    ...
    async def test_no_update_when_hash_unchanged(self, conn) -> None:
        from pipeline import replace_model_specs, upsert_manufacturer, upsert_model, upsert_series

        mfr_id = await upsert_manufacturer(conn, {"name": "HashCo", "slug": "hashco", "url": "/tractors/hashco/"})
        series_id = await upsert_series(conn, mfr_id, {"name": "H Series", "slug": "h-series", "models": []})
        ...
        specs = [{"group": "Engine", "key": "HP", "value": "50", "unit": "hp", "display_order": 0}]
        await replace_model_specs(conn, model_id, specs, test_hash)

        # Second upsert with same hash — should return needs_update=False
        _, needs_update = await upsert_model(conn, mfr_id, series_id, {**model_data, "content_hash": test_hash})
...
class TestReplaceModelSpecs:
    async def test_deletes_old_specs_before_insert(self, conn) -> None:
        ...
        mfr_id = await upsert_manufacturer(conn, {"name": "SpecCo", "slug": "specco", "url": "/tractors/specco/"})
        series_id = await upsert_series(conn, mfr_id, {"name": "S Series", "slug": "s-series", "models": []})
        model_data = {
            "name": "SpecCo S1",
            "slug": "specco-s1",
            "production_start_year": None,
        ...
        first_specs = [{"group": "Engine", "key": "HP", "value": "50", "unit": "hp", "display_order": 0}]
        await replace_model_specs(conn, model_id, first_specs, "hash_v1".ljust(64, "0"))

        second_specs = [
            {"group": "Engine", "key": "HP", "value": "60", "unit": "hp", "display_order": 0},
            {"group": "Engine", "key": "Cylinders", "value": "4", "unit": None, "display_order": 1},
        ]
        await replace_model_specs(conn, model_id, second_specs, "hash_v2".ljust(64, "0"))

        rows = await conn.fetch("SELECT key, value FROM model_specifications WHERE model_id = $1", model_id)
        assert len(rows) == 2
